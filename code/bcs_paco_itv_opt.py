...
if __name__ == '__main__':
    ...
    for iter in range(maxiter):
        ...
        #
        prev_cost = cost
        cost = (np.sum(np.abs(np.dot(Y,D.T))) + np.dot(dU.ravel(),U.ravel()) + (0.5/tau)*np.linalg.norm(dU,'fro')**2)/np.prod(dU.shape) 
        dcost = prev_cost - cost
        nY = np.linalg.norm(Y,'fro')+1e-10
        ndU = np.linalg.norm(dU,'fro') / nY # / np.linalg.norm(U,'fro')
        dY = np.linalg.norm(Y - prevY, 'fro') / nY
        dZ = np.linalg.norm(Z - prevZ, 'fro') / nY
        if not iter % 10:
            Xerr = np.linalg.norm(Y - Xtrue, 'fro') / nY
            E    = np.dot(Y, P.T)-B
            nE   = np.linalg.norm(E,'fro')
            merr = nE / np.prod(E.shape) 
            ...
            dt = (time.time() - t0)
            print(f"iter={iter:05d} tau={tau:8.5f} dY={dY:8.5f} dZ={dZ:8.5f} dU={ndU:8.5f} dcost={dcost:8.5f} merr={merr:8.5f} IMAGE: MSE={Ierr:8.5f} PSNR={psnr:8.5f} dt={dt:8.1f}s")
            stats.append([iter,tau,dY,dZ,ndU,cost,merr,Ierr,psnr])
            plt.imsave(os.path.join(args.outdir,f"iter{iter:04d}.png"),Irec,cmap=cm.gray)
            if dY < args.outer_tol and dZ < args.outer_tol:
    ...
    plt.imsave(os.path.join(args.outdir,'recovered.png'),Irec,cmap=cm.gray)
