...
def prox_f_block(Di, P, b, w, args, y0=None):
    '''
    Computes the proximal operator for a single block
    The problem to be solved is:

    y* = arg min ||y||_wp + 1/(2tau)|| y - w ||^2_2 s.t. Ay = b
    A = PDi
    ...
    if ImAtAAtiA is None:
        ...
        h = int(np.sqrt(len(w)))
        aux = np.ones(h)# 1.0/np.power(0.7,np.arange(h))
        WW = np.outer(aux,aux)
        WW = WW.ravel()
        print(WW)
    ...
    while iter < args.inner_maxiter:
        ...
        fcand = np.dot(WW,np.abs(ycand)) + (0.5/taup)*np.sum((ycand-w)**2)
...
def prox_f(Di,P,B,W,args,Y0=None):
    ...
    for j in range(n):
        ...
        if Y0 is None:
            ...
            y0j = Y0[j,:]
        Y[j,:] = prox_f_block(Di, P, bj, wj, args, y0j)
    return Y
...
if __name__ == '__main__':
    ...
    K = P.shape[1]
    print("Number of samples per block =",K)
    print("Total number of samples",n*K)
    print("Total number of pixels",n*K/npixels)
    B = np.dot(Xtrue,P)
    nsamples = n*K
    cratio   = n*K / npixels
    ...
    plt.show()
    mosaic = util.dictionary_mosaic(Di, 2, 0.25)
    plt.imshow(mosaic, cmap=cm.gray)
    plt.show()
    #
    ...
    # B*Pt = X*PPt
    # BPt(PPt)i = X
    PtPPti = np.dot(P.T,np.linalg.inv(np.dot(P, P.T)+0.1*np.eye(P.shape[0]))) # a little Ridge reg.
    ...
    for iter in range(1,args.maxiter):
        ...
        np.copyto(prevZ, Z)
        Z[:] = prox_g(Y + U, tau)
        Irec = patmap.stitch(Z, width, stride, nrows, ncols, Irec)
        #
