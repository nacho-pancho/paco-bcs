...
def plot_results(rate,widths,overlaps,z25,z50,z75,best):
    ...
    iov = (1/overlaps)
    s25 = 1000*(z25-0.6)/0.6
    s50 = 1000*(z50-0.6)/0.6
    s75 = 1000*(z75-0.6)/0.6
    imax   = np.argmax(z50)
    # mark best
    plt.scatter(widths[imax],iov[imax],marker="o",s=s75[imax]+300,facecolors="none",vmin=0.6,vmax=1,alpha=1,edgecolors="black",cmap=comap)
    plt.yscale('log')
    plt.xscale('log')
    ...
    # show all
    plt.scatter(widths,iov,marker="o",s=s75,c=z50,vmin=0.6,vmax=1,alpha=0.33,edgecolors=None,cmap=comap)
    plt.scatter(widths,iov,marker="o",s=s50,c=z50,vmin=0.6,vmax=1,alpha=0.5,edgecolors="black",cmap=comap)
    plt.scatter(widths,iov,marker="o",s=s25,c=z50,vmin=0.6,vmax=1,alpha=1,edgecolors=None,cmap=comap)
    plt.colorbar()
    plt.title(f"SSIM for subsampling rate {10*rate}%")
...
if __name__ == '__main__':
    ...
    for i in range(n2):
        S2[i, :-3] = S[i * nseeds, :-4]
        samples = S[i * nseeds:(i + 1) * nseeds, -1]  # SSIM for all seeds
    ...
    for i in range(n3):
        ...
        samples = S[i * nseeds*nimages:(i + 1) * nseeds*nimages, -1]  # SSIM for  7 seeds and 7 images
    ...
    plt.ylabel('SSIM')
    plt.title('SSIM at width=32px, overlap=2px')
    ...
    for r in range(1,6):
        ...
        for img in images:
            ...
            ssim = Si[:,-1]
            p25 = np.round(np.percentile(ssim,25),2)
            p50 = np.round(np.percentile(ssim,50),2)
            p75 = np.round(np.percentile(ssim,75),2)
